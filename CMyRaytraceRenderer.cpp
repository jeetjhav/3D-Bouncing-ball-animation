		...
		for (int c = 0; c < m_rayimagewidth; c++)
		{
			...
			if (m_intersection.Intersect(ray, 1e20, NULL, nearest, t, intersect))
			{
				...
				if (material != NULL)
				{
					...
					CGrPoint s = Normalize3(light.m_pos);
					CGrPoint v = Normalize3(intersect);
					CGrPoint n = {1., 1., 1.}; // TODO
					CGrPoint h = Normalize3(v + s);

					float diffuse_x = material->Ambient(0) + material->Diffuse(0) * lightInt * max(0., Dot3(n, s));
					float diffuse_y = material->Ambient(1) + material->Diffuse(1) * lightInt * max(0., Dot3(n, s));
					float diffuse_z = material->Ambient(2) + material->Diffuse(2) * lightInt * max(0., Dot3(n, s));

					float spec_x = material->Specular(0) * pow(max(0., Dot3(n, s)), 32.);
					float spec_y = material->Specular(1) * pow(max(0., Dot3(n, s)), 32.);
					float spec_z = material->Specular(2) * pow(max(0., Dot3(n, s)), 32.);


					CGrPoint color = { diffuse_x + spec_x, diffuse_y + spec_y, diffuse_z + spec_z };
