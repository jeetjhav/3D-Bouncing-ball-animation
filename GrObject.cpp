...
// 
void CGrComposite::Sphere(double cx, double cy, double cz, double radius, int slices, int stacks, CGrTexture* p_texture)
{
    CGrPtr<CGrPolygon> poly;
    double theta, phi;
    double x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4;
    double M_PI = 4 * atan(1);
    // Iterate through the stacks (latitude divisions)
    for (int i = 0; i < stacks; i++)
    {
        theta = M_PI * (i / (double)(stacks));             // Latitude angle
        double theta_next = M_PI * ((i + 1) / (double)(stacks)); // Next latitude angle

        // Iterate through the slices (longitude divisions)
        for (int j = 0; j < slices; j++)
        {
            phi = 2 * M_PI * (j / (double)(slices)); // Longitude angle

            // Calculate the vertices of the current quad (this is a quadrilateral on the sphere)
            x1 = cx + radius * sin(theta) * cos(phi);
            y1 = cy + radius * cos(theta);
            z1 = cz + radius * sin(theta) * sin(phi);

            x2 = cx + radius * sin(theta) * cos(phi + 2 * M_PI / slices);
            y2 = cy + radius * cos(theta);
            z2 = cz + radius * sin(theta) * sin(phi + 2 * M_PI / slices);

            x3 = cx + radius * sin(theta_next) * cos(phi + 2 * M_PI / slices);
            y3 = cy + radius * cos(theta_next);
            z3 = cz + radius * sin(theta_next) * sin(phi + 2 * M_PI / slices);

            x4 = cx + radius * sin(theta_next) * cos(phi);
            y4 = cy + radius * cos(theta_next);
            z4 = cz + radius * sin(theta_next) * sin(phi);

            // Create a polygon for the current quad
            poly = new CGrPolygon;
            poly->AddNormal3d((x1 + x2 + x3 + x4) / 4, (y1 + y2 + y3 + y4) / 4, (z1 + z2 + z3 + z4) / 4);
            poly->AddVertex3d(x1, y1, z1);
            poly->AddVertex3d(x2, y2, z2);
            poly->AddVertex3d(x3, y3, z3);
            poly->AddVertex3d(x4, y4, z4);

            if (p_texture)
            {
                poly->Texture(p_texture);
                poly->AddTex2d(j / (double)slices, i / (double)stacks);
                poly->AddTex2d((j + 1) / (double)slices, i / (double)stacks);
                poly->AddTex2d((j + 1) / (double)slices, (i + 1) / (double)stacks);
                poly->AddTex2d(j / (double)slices, (i + 1) / (double)stacks);
            }
            Child(poly);
        }
    }
...
//Cylinder
void CGrComposite::Cylinder(double cx, double cy, double cz, double radius, double height, int slices, CGrTexture* p_texture)
{
    CGrPtr<CGrPolygon> poly;
    double M_PI = 4 * atan(1);
    double halfHeight = height / 2.0;
    double topY = cy + halfHeight;
    double bottomY = cy - halfHeight;
    ...
    for (int i = 0; i < slices; i++)
    {
        ...
        double x1 = cx + radius * cos(theta1);
        double z1 = cz + radius * sin(theta1);
        double x2 = cx + radius * cos(theta2);
        double z2 = cz + radius * sin(theta2);

        poly = new CGrPolygon;
        poly->AddVertex3d(x1, bottomY, z1);
        poly->AddVertex3d(x1, topY, z1);
        poly->AddVertex3d(x2, topY, z2);
        poly->AddVertex3d(x2, bottomY, z2);
        poly->ComputeNormal();

        if (p_texture)
        {
            poly->Texture(p_texture);
            poly->AddTex2d(i / (double)slices, 0);
            poly->AddTex2d(i / (double)slices, 1);
            poly->AddTex2d((i + 1) / (double)slices, 1);
            poly->AddTex2d((i + 1) / (double)slices, 0);
        }
    ...
    for (int i = 0; i < slices; i++)
    {
        double theta1 = 2.0 * M_PI * i / slices;
        double theta2 = 2.0 * M_PI * (i + 1) / slices;
        ...
        CGrPtr<CGrPolygon> cap = new CGrPolygon;
        cap->AddNormal3d(0, 1, 0); // upward
        cap->AddVertex3d(cx, topY, cz);  // center
        cap->AddVertex3d(x1, topY, z1);
        cap->AddVertex3d(x2, topY, z2);

        if (p_texture)
        {
            cap->Texture(p_texture);
            cap->AddTex2d(0.5, 0.5);
            cap->AddTex2d(0.5 + 0.5 * cos(theta1), 0.5 + 0.5 * sin(theta1));
            cap->AddTex2d(0.5 + 0.5 * cos(theta2), 0.5 + 0.5 * sin(theta2));
        }
    ...
    for (int i = 0; i < slices; i++)
    {
        ...
        CGrPtr<CGrPolygon> cap = new CGrPolygon;
        cap->AddNormal3d(0, -1, 0); // downward
        cap->AddVertex3d(cx, bottomY, cz);  // center
        cap->AddVertex3d(x2, bottomY, z2);
        cap->AddVertex3d(x1, bottomY, z1);
